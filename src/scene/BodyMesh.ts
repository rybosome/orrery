...
export function createBodyMesh(options: CreateBodyMeshOptions): {
  ...
  // Default detail-noise seed should be stable but compact:
  // - prefer explicit `detailNoise.seed` when provided,
  // - otherwise hash `bodyId` when available,
  // - otherwise fall back to 0.
  const detailNoiseSeed = detailNoise?.seed ?? (options.bodyId ? stableHash01(options.bodyId) : 0)
        ...
        replacements: [
          ...
          },
          {
            ...
            replacement: [
              ...
              markerMap,
              '#ifdef USE_UV',
              '  float n = tspice_hash21( vUv * ( uCloudSwirlScale * 1.9 ) + vec2( uCloudSwirlPhase * 0.01 ) );',
              '  float k = ( n - 0.5 ) * uCloudSwirlAmount * 0.2;',
              '  diffuseColor.rgb *= clamp( 1.0 + k, 0.9, 1.1 );',
              '#endif',
            ].join('\n'),
