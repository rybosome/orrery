...
describe('defaultPlaybackRate', () => {
  ...
  it('returns 1d/s for the final third of the zoom curve', () => {
    expect(computeDefaultResumeRateSecPerSecForZoomSlider(67)).toBe(DEFAULT_RESUME_RATE_DAY_SEC_PER_SEC)
    expect(computeDefaultResumeRateSecPerSecForZoomSlider(100)).toBe(DEFAULT_RESUME_RATE_DAY_SEC_PER_SEC)
  })
  ...
  it('clamps zoom outside [0..100]', () => {
    ...
    expect(computeDefaultResumeRateSecPerSecForZoomSlider(1000)).toBe(DEFAULT_RESUME_RATE_DAY_SEC_PER_SEC)
  })

  it('keeps the weekly rate available as an explicit rate', () => {
    expect(DEFAULT_RESUME_RATE_WEEK_SEC_PER_SEC).toBe(7 * DEFAULT_RESUME_RATE_DAY_SEC_PER_SEC)
  })
  ...
  it('treats the 1/3 and 2/3 thresholds as inclusive of the next segment', () => {
    ...
    expect(computeDefaultResumeRateSecPerSecForZoomSlider(twoThirds)).toBe(DEFAULT_RESUME_RATE_DAY_SEC_PER_SEC)
  })
})
